...
```mermaid
flowchart LR
  subgraph p1["① ローカル PC（STDIO）"]
    A["Claude Desktop 等"] -->|STDIO| B["npx mcp-server\n📍PC上で動く"]
  end
  subgraph p2["② サーバーホスト型（StreamableHTTP）"]
    C["WxO Agent"] -->|HTTPS| D["MCP Server\n📍自前でホスト\nRailway 等"]
  end
  subgraph p3["③ WxO STDIO（今回）"]
    E["WxO Agent"] -->|"STDIO（内部）"| F["npx mcp-server\n📍WxO クラウド内\nPC不要・公開URL不要"]
  end
  B -->|TCP| PG[("PostgreSQL")]
  D -->|TCP| PG
  F -->|TCP| PG
...
**① ローカル PC**: Claude Desktop 等が手元の PC で `npx` を起動します。WxO エージェントからは利用できません。

**② サーバーホスト型**: MCP サーバーを公開 URL でホストし、WxO から HTTPS で接続します。サーバーの用意・運用が必要です。

**③ WxO STDIO（今回）**: WxO クラウド内で `npx` を起動します。PC も公開サーバーも不要です。
